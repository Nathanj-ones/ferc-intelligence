...
import { backendCredential, backendPaths } from './backend-config.mjs';
...
export class LocalRefresh {
  constructor(root, options = {}) {
    ...
    this.project = root;
    this.backend = backendPaths(root).backend;
    this.child = null;
    ...
    this.state = {
      ...
      phase: 'Ready',
      message:
        'Pull current source data, then validate. This can take several minutes.',
    };
    this.active = null;
  }
  async init() {
    ...
    if (this.active && !GENERATION.test(this.active.generationId))
      throw new Error('Invalid local snapshot pointer');
      ...
      this.state =
        previous.status === 'running'
          ? {
              ...
              phase: 'Interrupted',
              message:
                'The local server stopped during refresh. The last good snapshot is unchanged; you can retry.',
            }
          : previous;
    return this;
  }
  async status() {
    return {
      enabled: true,
      credentialConfigured: Boolean(await backendCredential(this.project)),
      ...this.state,
  ...
  async start() {
    ...
    this.state = {
      ...
      phase: 'Preparing',
      message: 'Preparing an isolated local working copy…',
    };
  ...
  async execute(command, args, env, label) {
    ...
    await this.update({
      phase: label,
      message: `${label}… The current snapshot remains available.`,
    });
    await new Promise((resolve, reject) => {
      ...
      child.once('close', (code) => {
        ...
        if (code === 0) resolve();
          ...
          console.error(
            `[FERC local refresh] ${label} did not complete (exit ${code ?? 'timeout/interrupted'}). No new snapshot activated.`,
          );
  ...
  async perform() {
    ...
    await this.update({});
    const key = await backendCredential(this.project);
    if (!key) {
      await this.update({
        status: 'failed',
        phase: 'Configuration',
        message:
          'FERC_API_KEY is missing. Configure it in the repository .env file or server environment, then retry.',
      });
      return;
    }
    const python = backendPaths(this.project).python;
    ...
    if (!(await optionalJson(path.join(work, 'initialized.json')))) {
      try {
        await fs.access(
          path.join(this.backend, 'staging/operating_assets.sqlite'),
        );
      } catch {
        await this.update({
          status: 'failed',
          ...
          message:
            'Backend starting data is not installed. Run npm run setup:backend in this repository, then retry.',
        });
        return;
      }
      ...
      await this.run(
        ...
        env,
        'Copying reviewed database',
      );
      await fs.chmod(temporaryDb, 0o600);
      await fs.rename(temporaryDb, db);
      await this.update({
        phase: 'Copying source cache',
        message:
          'First refresh: preparing a private cache (about 2 GB of local working data).',
      });
      await fs.cp(path.join(this.backend, 'source_cache'), cache, {
        recursive: true,
      ...
      const writableDirectories = async (directory) => {
        ...
        for (const entry of await fs.readdir(directory, {
          ...
          if (entry.isSymbolicLink())
            throw new Error('Source-cache symlinks are not supported');
          if (entry.isDirectory())
            await writableDirectories(path.join(directory, entry.name));
        }
      };
      await writableDirectories(cache);
      await atomicJson(path.join(work, 'initialized.json'), {
    ...
    const cli = (command, args, label) =>
      this.run(
        python,
        [path.join(this.backend, 'run.py'), command, ...common, ...args],
        env,
        label,
      );
    ...
    await this.run(
      python,
      [
        path.join(this.backend, 'build_field_status.py'),
        '--db',
        db,
        '--out',
      ...
      env,
      'Recomputing field statuses',
    );
    await cli('export', ['--offline'], 'Building source snapshot');
    await cli('validate', ['--offline'], 'Validating source snapshot');
    const receipt = await readJson(
      path.join(output, 'publication_receipt.json'),
    );
    if (!GENERATION.test(receipt.generation_id))
      throw new Error('Invalid publication generation');
    const presentation = path.join(this.root, 'data');
    await this.run(
      process.execPath,
      [path.join(this.project, 'scripts/sync-ferc-backend.mjs')],
      {
        ...env,
        FERC_BACKEND_ROOT: output,
        FERC_FRONTEND_DATA_ROOT: presentation,
    ...
    if (pointer.generationId !== receipt.generation_id)
      throw new Error('Publication pointer mismatch');
    const active = {
    ...
    await this.update({
      status: 'succeeded',
      phase: 'Complete',
      completedAt: active.refreshedAt,
      message:
        'Source refresh and validation passed. The new snapshot is ready.',
    });
  }
}
    ...
    async configureServer(server) {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        const match = pathname.match(
          /^\/data\/ferc\/generations\/([a-f0-9]{64})\/(.+)$/,
        );
        if (!api && !match) return next();
        if (!trustedLocalRequest(req, req.method === 'POST'))
          return json(res, 403, {
            error: 'Only same-origin localhost requests are allowed.',
          });
        try {
          if (pathname === '/api/local-ferc/status' && req.method === 'GET')
            return json(res, 200, await manager.status());
          if (pathname === '/api/local-ferc/refresh' && req.method === 'POST') {
            ...
            if (body.trim() !== '{}')
              return json(res, 400, { error: 'Refresh takes no parameters.' });
            if (!(await manager.start()))
              return json(res, 409, await manager.status());
            return json(res, 202, await manager.status());
          }
          if (api)
            return json(res, 404, { error: 'Unknown local refresh route.' });
          ...
          if (
            ...
            return json(res, 404, { error: 'Not in snapshot manifest.' });
          const file = path.resolve(base, relative);
          if (
            !(await fs.realpath(file)).startsWith(
              `${await fs.realpath(base)}${path.sep}`,
            )
          )
            return json(res, 403, { error: 'Invalid snapshot path.' });
          res.writeHead(200, {
            'Content-Type': relative.endsWith('.gz')
              ? 'application/gzip'
              : 'application/json',
          ...
          if (!res.headersSent)
            json(res, 500, {
              error:
                'The local refresh service could not complete this request.',
            });
          else res.destroy();
        }
      });
